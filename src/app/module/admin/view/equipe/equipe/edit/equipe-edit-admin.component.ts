...
@Component({
  selector: 'app-equipe-edit-admin',
  templateUrl: './equipe-edit-admin.component.html'
})
export class EquipeEditAdminComponent implements OnInit {
 ...
 public edit(): void {
   ...
   public addMembreEquipes() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.membreEquipesElement.id == null){
                this.item.membreEquipes.push(this.membreEquipesElement);
            }else{
                const index = this.item.membreEquipes.findIndex(e => e.id == this.membreEquipesElement.id);
                this.item.membreEquipes[index] = this.membreEquipesElement;
            }
   ...
   public async openCreateTypeEquipe(typeEquipe: string) {
        const isPermistted = await this.roleService.isPermitted('TypeEquipe', 'edit');
        if (isPermistted) {
